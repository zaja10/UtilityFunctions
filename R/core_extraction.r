...
#'     \itemize{
#'        \item Note: VAF is defined as the genetic variance explained by the factors relative to the \strong{total} genetic variance (\eqn{\Lambda\Lambda' + \Psi}).
#'     }
...
fa.asreml <- function(model, classify, psi_term = NULL, ...) {
  ...
  if (!is.null(model$vparameters.con)) {
    ...
    if (any(con_codes %in% c("B", "F"))) {
      n_bound <- sum(con_codes %in% c("B", "F"))
      warning(sprintf("Model has %d parameters at boundary (B) or fixed (F). FAST indices may be unstable.", n_bound))
    }
  }
  ...
  eigen_vals <- eigen(G_est, symmetric = TRUE, only.values = TRUE)$values
  if (any(eigen_vals <= 1e-8)) {
    warning("Estimated G matrix is not Positive Definite (Eigenvalues <= 0). Heritability estimates may be invalid.")
  }
