...
We fit a Factor Analytic model of order 2 (FA2) to the Variety × Environment interaction.
...
model_real <- asreml(
  ...
  data = df,
  trace = FALSE
) # Suppress iteration output for vignette
...
# [NEW] Validate Genomic Rigor
# Verify the GRM is properly scaled (mean diag ~ 1) for valid Oakey estimates
validate_grm(my_grm)
...
# [NEW] Model Audit
# Check if the model respects the spatial design (Row/Col terms present?)
audit_model_terms(mod_genomic)
```
...
plot_spatial(mod_genomic, row = "Row", col = "Column", attribute = "residuals")
```
...
# Part 3: Biometric Rigor & Site Metrics

New in version 1.2: rigorous site-specific quality metrics that handle heterogeneous genetic variances.

## 1. Site Repeatability (Model-Based)

We calculate the rigorous Reliability ($R$) for each site. Unlike simple variance ratios, this method uses `predict()` to obtain the **Prediction Error Variance (PEV)** of the BLUPs, providing a true measure of site quality that accounts for experimental design imbalance and specific variances.

$$R_{site} = 1 - \frac{\text{PEV}_{avg}}{V_g}$$

```{r}
# Calculates rigorous R using PEV
# Automatically handles predict() calls per site
rep_df <- compare_repeatability(mod_genomic, results_g)
print(head(rep_df))
```

## 2. Heritability & Reliability Comparison

We compare standard phenotypic heritability (Cullis) against genomic reliability (Oakey). The "Oakey" method is the gold standard for genomic trials as it incorporates the GRM structure.

```{r}
# Compare methods. Note: 'classify' is auto-detected from results_g
h2_df <- compare_h2(mod_genomic, results_g,
  grm = my_grm,
  methods = c("Cullis", "Oakey")
)

print(head(h2_df))
```

## 3. High-Throughput Visualizations

We can visualize the Genetic Variances ($V_g$) directly to see which sites have the strongest signal, and use clustered heatmaps to find Mega-Environments.

```{r}
# A. Genetic Variance Barplot
plot(results_g, type = "h2")

# B. Clustered Genetic Correlation Heatmap
# (Hierarchical clustering groups similar environments)
plot(results_g, type = "heatmap")
```


# Part 4: Specialized Models (Reduced Rank)

For advanced users fitting Reduced Rank models where specific variances are modeled separately (e.g., via `diag()`), `fa.asreml` supports flexible extraction.

## Reduced Rank Extraction

When fitting a model like:
`random = ~ rr(Site, 2):Gen + diag(Site):Gen`

You must explicitly tell `fa.asreml` where to find the specific variances using `psi_term`.

```{r}
# Example RR Model
model_rr <- asreml(fixed = Yield ~ Site, 
                   random = ~ rr(Site, 2):Gen + diag(Site):Gen, 
                   ...)

# Extraction
# We pass psi_term to ensure the diagonal variances are captured
results_rr <- fa.asreml(model_rr, 
                        classify = "rr(Site, 2):Gen", 
...
The toolkit matches the sites in the Reduced Rank loading matrix with the sites in the diagonal variance structure to correctly reconstruct the Genetic Covariance Matrix ($G = \Lambda\Lambda' + \Psi$).
